...
function valid(scheme,plan,option,schemeNavMap){
        if(!(plan in schemeNavMap[scheme]["plans"] )){
            ...
            let speechText = `${plan} does not exist on the ${scheme}. but `
            
            planInScheme.forEach(function(plan,index){
                const tempOption = Object.keys(schemeNavMap[scheme]["plans"][plan])
                tempOption.forEach((option)=>{
                    speechText += `The n.a.v for scheme ${scheme} plan ${plan} and option ${option} is ${schemeNavMap[scheme]['plans'][plan][option]["nav"]} rupees dated ${schemeNavMap[scheme]['plans'][plan][option]["date"]}`
                })
...
const isValidAndResponse = valid("Aditya Birla Sun Life Dividend Yield Fund".toUpperCase(),`MAC`,'REGULAR',schemeNavMap)
